...
fn gen_uint64(n: usize) -> RecordBatch {
    ...
    RecordBatch::try_new(
        Arc::new(Schema::new(vec![Field::new("val", DataType::UInt64, false)])),
        vec![Arc::new(arr)],
    ).unwrap()
}
...
fn gen_float64(n: usize) -> RecordBatch {
    ...
    RecordBatch::try_new(
        Arc::new(Schema::new(vec![Field::new("val", DataType::Float64, false)])),
        vec![Arc::new(arr)],
    ).unwrap()
}
...
fn gen_string(n: usize) -> RecordBatch {
    ...
    let arr = StringArray::from((0..n).map(|i| strings[i % 100].as_str()).collect::<Vec<_>>());
    RecordBatch::try_new(
...
fn gen_date32(n: usize) -> RecordBatch {
    ...
    let arr = Date32Array::from((0..n as i32).map(|i| 18262 + (i % 3650)).collect::<Vec<_>>());
    RecordBatch::try_new(
        Arc::new(Schema::new(vec![Field::new("val", DataType::Date32, false)])),
        vec![Arc::new(arr)],
    ).unwrap()
}
...
fn gen_timestamp(n: usize) -> RecordBatch {
    ...
    let arr = TimestampMicrosecondArray::from(
        (0..n as i64).map(|i| 1_700_000_000_000_000 + i * 1_000_000).collect::<Vec<_>>()
    );
    RecordBatch::try_new(
        Arc::new(Schema::new(vec![
            Field::new("val", DataType::Timestamp(arrow_schema::TimeUnit::Microsecond, None), false)
        ])),
        vec![Arc::new(arr)],
    ).unwrap()
}
...
fn gen_struct(n: usize) -> RecordBatch {
    ...
    let name_arr = StringArray::from((0..n).map(|i| format!("user_{}", i % 1000)).collect::<Vec<_>>());
    ...
    let struct_arr = StructArray::from(vec![
        ...
        (Arc::new(Field::new("name", DataType::Utf8, false)), Arc::new(name_arr) as ArrayRef),
        (Arc::new(Field::new("age", DataType::Int32, false)), Arc::new(age_arr) as ArrayRef),
    ]);
    RecordBatch::try_new(
        Arc::new(Schema::new(vec![Field::new("val", DataType::Struct(fields), false)])),
...
fn gen_list(n: usize) -> RecordBatch {
    ...
    RecordBatch::try_new(
        Arc::new(Schema::new(vec![
            Field::new("val", DataType::List(Arc::new(Field::new("item", DataType::Int64, true))), false)
        ])),
        vec![Arc::new(arr)],
    ).unwrap()
}
...
fn gen_map(n: usize) -> RecordBatch {
    ...
    RecordBatch::try_new(
        Arc::new(Schema::new(vec![
            Field::new("val", arr.data_type().clone(), false)
        ])),
        vec![Arc::new(arr)],
    ).unwrap()
}
...
fn gen_mixed(n: usize) -> RecordBatch {
    ...
    let revenue = Float64Array::from((0..n).map(|i| (i as f64) * 37.5 % 99999.0).collect::<Vec<_>>());
    let region = StringArray::from((0..n).map(|i| match i % 4 {
        0 => "North", 1 => "South", 2 => "East", _ => "West"
    }).collect::<Vec<_>>());
    let ts = TimestampMicrosecondArray::from(
        (0..n as i64).map(|i| 1_700_000_000_000_000 + i * 1_000).collect::<Vec<_>>()
    );
    ...
    RecordBatch::try_new(
        ...
        vec![
            Arc::new(user_id), Arc::new(revenue),
            Arc::new(region), Arc::new(ts), Arc::new(active),
        ],
    ).unwrap()
}
...
fn bench_flux(batch: &RecordBatch, raw: usize, profile: CompressionProfile) -> BenchResult {
    ...
    if !all_supported {
        return BenchResult {
            format: name.into(), size: 0, ratio: 0.0,
            comp_mbs: 0.0, decomp_mbs: 0.0, supported: false,
        };
    }
    ...
    let t1 = Instant::now();
    let _ = reader.decompress_file_all(std::path::Path::new(tmp)).unwrap();
    let d_ms = t1.elapsed().as_secs_f64() * 1000.0;

    let _ = fs::remove_file(tmp);

    BenchResult {
        format: name.into(), size: bytes.len(),
        ratio: raw as f64 / bytes.len() as f64,
        comp_mbs: mbs(raw, c_ms), decomp_mbs: mbs(raw, d_ms),
        supported: true,
    }
}
...
fn bench_parquet(batch: &RecordBatch, raw: usize, codec: &str) -> BenchResult {
    ...
    let pr = ParquetRecordBatchReaderBuilder::try_new(file).unwrap().build().unwrap();
    ...
    BenchResult {
        format: format!("Parquet ({codec})"), size,
...
fn bench_orc(batch: &RecordBatch, raw: usize) -> BenchResult {
    ...
    if result.is_err() {
        let _ = fs::remove_file(tmp);
        return BenchResult {
            format: "ORC (zlib)".into(), size: 0, ratio: 0.0,
            comp_mbs: 0.0, decomp_mbs: 0.0, supported: false,
        };
    }
...
pub fn cmd_dtype_bench(rows: usize) -> Result<()> {
    ...
    for (dtype, batch) in &tests {
        ...
        println!("── {dtype} ({}, {} cols) ──", human(raw), batch.num_columns());
        println!(
            "  {:<24} {:>10} {:>8} {:>10} {:>10}",
        ...
        for r in &results {
            if r.supported {
                println!(
                    "  {:<24} {:>10} {:>7.1}x {:>9.0} {:>9.0}",
                    r.format, human(r.size), r.ratio, r.comp_mbs, r.decomp_mbs,
                );
            } else {
                println!(
                    "  {:<24} {:>10} {:>8} {:>10} {:>10}",
