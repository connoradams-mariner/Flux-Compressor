...
pub(crate) fn filter_batch(batch: &RecordBatch, mask: &BooleanArray) -> FluxResult<RecordBatch> {
    let filtered_cols: Vec<ArrayRef> = batch
        ...
        .iter()
        .map(|c| {
            arrow::compute::filter(c, mask).map_err(|e| FluxError::Internal(format!("filter: {e}")))
        })
...
mod tests {
    ...
    fn scalar_type_mismatch_errors() {
        ...
        let err = ScalarValue::Int(1)
            .to_array(&DataType::Utf8, 3)
            .unwrap_err();
    ...
    fn merge_update_from_source_updates_matched_rows() {
        ...
        let source = RecordBatch::try_new(schema, vec![Arc::new(ids), Arc::new(scores)]).unwrap();
